...
const waitForReconnectText = async (
	...
	throw new Error(`Timed out waiting for ${pattern}. Latest: ${latest || '<empty>'}`);
};
...
const startedServer = process.env.VOICE_SMOKE_USE_EXISTING_SERVER === '1'
	? null
	: spawnProcess(['bun', 'run', 'dev'], { PORT: '3004' });
const startedChrome = process.env.VOICE_SMOKE_USE_EXISTING_CHROME === '1'
	? null
	: spawnProcess([
			'google-chrome',
			'--headless=new',
